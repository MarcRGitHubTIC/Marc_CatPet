...
public class Mascota extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        RecyclerView recyclerView = findViewById(R.id.recycler_mascota);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        List<MascotaItem> items = new ArrayList<>();
        items.add(new MascotaItem("Vacunas", "Consulta el calendario de vacunación", R.drawable.logo));
        items.add(new MascotaItem("Citas médicas", "Historial y próximas citas", R.drawable.logo));
        items.add(new MascotaItem("Dieta", "Recomendaciones de alimentación", R.drawable.logo));

        MascotaAdapter adapter = new MascotaAdapter(items);
        recyclerView.setAdapter(adapter);


        Log.d("DEBUG", "Actividad Mascota creada");
